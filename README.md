...
flowchart TB
    subgraph A["PILLAR A — Extraction Pipeline"]
        ...
        ING --> EXT[Extraction<br/>runs prompts/extraction_v5.1.md<br/>via Anthropic API]
...
The pinned default is **`extraction_v5.1`** — `extraction_v5` plus one *additive* output (the `text_endpoints` side-table, see below). The core 26-column behavior is unchanged; this is the single, deliberate, narrow exception to the "prompt is a black box" rule.

```python
# extraction/prompt_loader.py
def load_prompt(version: str = "extraction_v5.1") -> PromptBundle:
    """Load prompts/<version>.md. Returns text + sha256 + version string.
    Raises if the file is missing — we never silently fall back to another version."""
    ...

# A run records exactly what it used:
#   prompt_runs.prompt_version = "extraction_v5.1"
#   prompt_runs.prompt_sha256  = sha256(prompts/extraction_v5.1.md)
