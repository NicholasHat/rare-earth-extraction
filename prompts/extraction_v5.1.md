...
# OUTPUT CONTRACT (extraction_v5.1 additive layer — do not remove)

## How you receive the paper
You are given the paper **as a PDF document in this conversation**, not a file path. The `pdfplumber`,
`numpy`, and morphology snippets in Steps 2–6 are **conceptual guidance for how to read a figure** —
you cannot execute them here. Read the rendered figure pages **visually**: identify the legend, read
the axis tick labels, and digitise each element's curve point-by-point by eye, applying the same
calibration, colour-vs-monochrome, overlap-resolution, and validation logic those steps describe.
