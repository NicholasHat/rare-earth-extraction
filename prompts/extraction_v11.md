...
- Essentially one large image and ~0 drawings → **raster** → render it (300 dpi is enough for the
  toolkit) and read pixels with the SANDBOX TOOLKIT.
...
which picks linear vs log10 for you and returns `cal.pixel_to_data(px)`.
...
- filled square/circle/diamond/star → `page.curves` with `fill=True` (raster: the toolkit's
...
# OUTPUT CONTRACT (extraction_v11 additive layer — do not remove)
...
**When an authoritative page also carries a `DIGITIZED CURVE DATA` block, skip calibration and
clustering entirely for the series it covers** — go straight from that data to a finished row:
- The block is `{"<group_key>": [[x, y], ...], ...}`, one entry per legend colour, already
  calibrated against this figure's own axis ticks (Step 3 is done for these series). Do not
  re-derive the pixel→data mapping, and do not re-cluster markers to get these points — both
  would just reproduce numbers you already have.
- Your only remaining work for these series is exactly Step 4's legend step (map each `group_key`
  hex colour to the element/condition it represents — re-derive this from the legend every time,
  same as always) and Step 7/8's unit conversions and experimental-constants columns. Then go
  straight to Step 9.
- Step 6 (resolve overlapping markers; validate against stated text end-points) still applies to
  these points same as any other digitised series — the block gives you raw coordinates, not a
  finished, deduplicated, or endpoint-verified row set.
- A `group_key` present in the counts sentence but absent from the `DIGITIZED CURVE DATA` block (or
  vice versa) means the deterministic pass couldn't calibrate that series — fall back to full visual
  digitisation for it, same as an estimate/raster page.
- This shortcut never applies to estimate/raster pages, and never covers monochrome
  (marker-shape-only) series even on an authoritative page — the deterministic pass only ever
  supplies coordinates for coloured filled markers, so shape-only series always need your own full
  Steps 3-5 treatment regardless of what else on that page is pre-digitised.
...
runs (render + look; blank text regions + detect + frame + ticks; calibrate + convert + assign
series), not twenty. Your judgement still does the parts code cannot: panel layout, where the legend
...
`pdfplumber` may not import in the sandbox and does not need to. Use your visual read of the page only
...
7. **Digitise the WHOLE curve, not the endpoints.** A typical "% Extraction vs. pH" series has ~15–20
